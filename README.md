...
│   ├── data.py                       # CSV loader; AppData singleton
│   └── routers/
│       ├── analysis.py               # /analysis/* endpoints
│       ├── series.py                 # /series and /series/{id}
│       └── speakers.py               # /speakers
├── frontend/                         # React + Vite dashboard (Phase 5)
│   └── src/
│       ├── pages/                    # Overview, SeriesExplorer, Findings, Speakers
│       └── components/               # NavBar, StatCard, ModelTable, SentimentLine
└── requirements.txt
```
...
Requires `data/processed/game_sentiment_agg.csv` and `data/processed/sentiment_game_joined.csv` (generated by `src/analysis/correlations.py`).

**Start the API:**
```bash
uvicorn api.main:app --reload
# Starts on http://localhost:8000
# Prints: Data loaded: 141 games, 10881 turns
```
...
**Available pages:**
- `/` - Overview: corpus stats, model comparison table, key findings
- `/series` - Series Explorer: per-series game-by-game sentiment arc with win/loss markers
- `/findings` - Research Findings: trajectory chart, series position chart, Pearson correlation table
- `/speakers` - Speaker Sentiment: ranked horizontal bar chart with adjustable min-turn filter

**API endpoints:**
```bash
curl localhost:8000/analysis/summary    # Corpus stats, model comparison, Pearson results
curl localhost:8000/analysis/trajectory # Sentiment by game position (1-7)
curl localhost:8000/analysis/series-position  # Home leads vs trails breakdown
curl localhost:8000/series              # All 30 series with labels
curl "localhost:8000/series/('BOS',%20'GSW')_2021"  # Game-by-game for one series
curl "localhost:8000/speakers?min_turns=50"          # Top speakers by sentiment
```
